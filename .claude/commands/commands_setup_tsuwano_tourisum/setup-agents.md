---
description: >
  9 個のサブエージェントを .claude/agents/ 配下に構築する。
  ...
  承認を得ながら作成する。
  各エージェントは Phase 4 で作成された Skill を参照する設計。
  /setup-skills の完了後に実行する。
allowed-tools: Read, Write, Glob, Bash(mkdir *), Bash(ls *), Bash(cat *)
---

# /setup-agents — サブエージェント群構築コマンド

> Phase 5 of 8. Let's think step by step.
...
Phase 4 完了を確認。「構築物の相互参照マップ」セクションから利用可能な Skill のリストを取得。
...
Phase 4 で作成された Skill が存在することを確認。サブエージェントはこれらを参照する。
...
`.steering/_setup-progress.md` の Phase 5 を完了マーク + 相互参照マップを更新:

```markdown
- [x] **Phase 5: /setup-agents** — サブエージェント群
