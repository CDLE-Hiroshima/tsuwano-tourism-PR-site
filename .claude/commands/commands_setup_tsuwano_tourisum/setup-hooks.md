...
description: >
  ...
  加えて SessionStart で動的情報表示。さらに hook 単独エラー（実行権限漏れ・構文エラー・
...
> Phase 7 of 8. Let's think step by step.
> Hook はエージェントループの外で決定論的に動作するスクリプト。
> 「人間が毎回チェックすべきこと」を Hook に任せる。
> **Hook は単独で壊れやすい（実行権限漏れ・構文エラー・依存コマンド不足・settings.json の綻び）。
> だから本コマンドは、hook を使う前に走らせる健康診断 `self-check.sh` も併せて構築する。**

## 設計原則: 3 層構成 (Preflight / Guard / Report) + 健康診断 (self-check)
...
### 健康診断: self-check.sh（hook を使う前の点検）

Hook は **単独で壊れやすい**。よくある単独エラー: (1) `chmod +x` 忘れで実行されない、(2) スクリプトの構文エラー、(3) `jq` などの依存コマンドが無い、(4) `settings.json` が壊れている / 存在しないスクリプトを参照している。これらは「hook が黙って動かない」状態を生み、気づきにくい。

そこで、hook 群を **実際に使う前に走らせる健康診断** `self-check.sh` を構築する:

- 各 hook スクリプトの **存在・実行権限・shebang・構文（`bash -n`）** を確認
- **依存コマンド**（git / bash / jq / lint・fmt ツール）の有無を確認
- **`settings.json` が妥当な JSON** で、参照する `.sh` がすべて実在するかを確認
- `preflight.sh` / `session-start.sh` を **安全に試走**（スモークテスト）して非0終了しないか確認
- 結果を `[self-check] OK/WARN/FAIL` で表示し、FAIL があれば終了コード 1

SessionStart で **最初に** 走らせることで「今セッションの hook は健康か」を毎回可視化し、hook 変更後の手動確認にも使える。
...
Phase 6 (`/setup-commands`) 完了を確認。されていない場合は中断し、先に `/setup-commands` を完了するよう通知。
...
settings.json で SessionStart Hook を登録（`bash` プレフィックスを付けて他 hook と形式を揃える）。**self-check.sh を先頭**に置き、その後に session-start.sh:
...
```json
{
  "hooks": {
    "SessionStart": [
      {
        "hooks": [
          { "type": "command", "command": "bash .claude/hooks/self-check.sh" },
...
### Step 8.5: self-check.sh（Hook 健康診断）の作成

他の hook をすべて作り終えたら、それらを **使う前に点検する** 健康診断スクリプトを作る。hook 単独エラー（実行権限漏れ・構文エラー・依存コマンド不足・settings.json の綻び）を早期に検知する。

`.claude/hooks/self-check.sh`:

```bash
#!/usr/bin/env bash
# self-check.sh — Hook 健康診断（hook を使う前に走らせる）
# 目的: hook スクリプトの単独エラー（実行権限漏れ・構文エラー・依存コマンド不足・
#       settings.json の綻び）を、実際に使う前に検出する。
# 使い方: bash .claude/hooks/self-check.sh   （SessionStart でも先頭で自動実行）
# 出力: [self-check] 各項目 OK/WARN/FAIL + 最後にサマリ
# 終了コード: FAIL があれば 1、なければ 0（WARN は 0）
...
FAIL=0; WARN=0; OK=0
say() { echo "[self-check] $1"; }

# 1. 依存コマンドの有無（必須: git / bash）
for c in git bash; do
    if command -v "$c" >/dev/null 2>&1; then
        OK=$((OK + 1))
    else
        say "FAIL: 必須コマンド '$c' が見つかりません"; FAIL=$((FAIL + 1))
    fi
done
# jq は settings.json の検証に使う（無ければ WARN で継続）
JQ_OK=1
if ! command -v jq >/dev/null 2>&1; then
    say "WARN: 'jq' が無いため settings.json の厳密検証をスキップします"; WARN=$((WARN + 1)); JQ_OK=0
fi

# 2. hook スクリプト個別チェック（存在・実行権限・shebang・構文）
if [ -d "$HOOK_DIR" ]; then
    for f in "$HOOK_DIR"/*.sh; do
        [ -e "$f" ] || continue
        name=$(basename "$f")
        [ "$name" = "self-check.sh" ] && continue  # 自分自身は除外
        [ -x "$f" ] || { say "WARN: $name に実行権限がありません（chmod +x を実行）"; WARN=$((WARN + 1)); }
        head -1 "$f" | grep -q '^#!' || { say "WARN: $name に shebang がありません"; WARN=$((WARN + 1)); }
        if bash -n "$f" 2>/dev/null; then
            OK=$((OK + 1))
        else
            say "FAIL: $name に構文エラーがあります（bash -n で検出）"; FAIL=$((FAIL + 1))
        fi
    done
else
    say "WARN: $HOOK_DIR が存在しません"; WARN=$((WARN + 1))
fi

# 3. settings.json の妥当性 + 参照スクリプトの実在
if [ -f "$SETTINGS" ]; then
    if [ "$JQ_OK" = "1" ]; then
        if jq empty "$SETTINGS" >/dev/null 2>&1; then
            OK=$((OK + 1))
            # settings.json が参照する .claude/hooks/*.sh がすべて実在するか
            MISSING_REFS=$(jq -r '.. | .command? // empty' "$SETTINGS" 2>/dev/null \
                | grep -oE '\.claude/hooks/[A-Za-z0-9_-]+\.sh' | sort -u \
                | while read -r ref; do [ -f "$REPO_ROOT/$ref" ] || echo "$ref"; done)
            if [ -n "$MISSING_REFS" ]; then
                for r in $MISSING_REFS; do
                    say "FAIL: settings.json が参照する $r が存在しません"; FAIL=$((FAIL + 1))
                done
            fi
        else
            say "FAIL: settings.json が不正な JSON です"; FAIL=$((FAIL + 1))
        fi
    fi
else
    say "WARN: settings.json がありません"; WARN=$((WARN + 1))
fi

# 4. スモークテスト（安全な入力で試走し、異常終了しないか）
for h in preflight.sh session-start.sh; do
    if [ -x "$HOOK_DIR/$h" ]; then
        if bash "$HOOK_DIR/$h" >/dev/null 2>&1; then
            OK=$((OK + 1))
        else
            say "WARN: $h の試走が非0終了しました（要確認）"; WARN=$((WARN + 1))
        fi
    fi
done
...
# サマリ
if [ "$FAIL" -gt 0 ]; then
    say "SUMMARY: FAIL=$FAIL WARN=$WARN OK=$OK — hook を使う前に修正してください"
    exit 1
elif [ "$WARN" -gt 0 ]; then
    say "SUMMARY: WARN=$WARN OK=$OK — 動作しますが要確認"
    exit 0
else
    say "SUMMARY: all hooks healthy (OK=$OK)"
    exit 0
fi
```
...
```bash
chmod +x .claude/hooks/self-check.sh
```

⚠️ **self-check.sh 自身は「常に exit 0 か FAIL 時のみ 1」** に留め、SessionStart をブロックしない設計にする（診断が原因でセッションが始まらない事態を避ける）。
⚠️ スモークテスト対象（`preflight.sh` / `session-start.sh`）は **副作用が無く引数不要** の hook に限る。Guard 系（PreToolUse）は本物のツール入力が要るのでここでは試走しない。

ユーザーに内容を表示して承認を得る。lint/fmt ツール（ruff / clippy / prettier 等）の存在確認を足したい場合は「1. 依存コマンドの有無」に追記する。
...
⚠️ **self-check.sh を SessionStart の先頭に置く**。他の hook より前に走らせることで、壊れた hook を「使う前」に検知できる。
...
0. **健康診断 (self-check)**: `bash .claude/hooks/self-check.sh` を実行すると `[self-check] SUMMARY: all hooks healthy (...)` が出るか。試しに 1 つの hook から実行権限を外す / 構文を壊す と `[self-check] WARN/FAIL` を検知し、直すと healthy に戻るか
1. **SessionStart**: `/clear` 後に self-check の要約に続いてプロジェクト情報が表示されるか
...
> - **健康診断**: self-check.sh は SessionStart の先頭にあるか? `bash -n`・実行権限・依存コマンド・settings.json 妥当性・参照スクリプト実在を確認しているか? SessionStart をブロックしない設計か?
...
> - 全 hook の出力が `[self-check]` / `[preflight]` / `[guard]` / `[fmt]` / `[stop]` プレフィックスで統一されているか?
...
```markdown
- [x] **Phase 7: /setup-hooks** — Hook 群（3 層 + 健康診断 self-check）
  - 完了日時: [YYYY-MM-DD HH:MM]
  - 作成 Hook:
    - SessionStart: .claude/hooks/self-check.sh（先頭で hook 健康診断）→ session-start.sh（動的情報表示）
  ...
  - 出力プレフィックス統一: [self-check] / [preflight] / [guard] / [fmt] / [stop]
  - settings.json: [新規作成 / 既存に追記]（全 hook を `"type": "command"` に統一）
  - 外部スクリプト:
    - .claude/hooks/self-check.sh（健康診断）
    - .claude/hooks/session-start.sh
    - .claude/hooks/preflight.sh
    - .claude/hooks/pre-edit-steering.sh
...
### Hook → Command 参照
- SessionStart Hook (self-check → session-start) → 全コマンドのセッション開始時に動作。先頭の self-check で壊れた hook を検知
...
設定した Hook: 5 種 + 健康診断 self-check

【健康診断】
0. SessionStart(先頭) → self-check.sh
   → hook を使う前に 実行権限/構文/依存コマンド/settings.json を点検
   → 出力: [self-check] SUMMARY: all hooks healthy ... / FAIL: ...
...
【Preflight 層】
2. UserPromptSubmit → preflight.sh
   → 毎ターン .steering/ 状態・git 状態・reimagine 適用状態を動的チェック
   → 出力: [preflight] task: ... / [preflight] git: ...

【Guard 層】
3. PreToolUse → pre-edit-steering.sh + pre-edit-banned.sh
   → 実装ファイル編集時に [guard] PASS or BLOCKED を明示
   → 出力: [guard] PASS: ... / [guard] BLOCKED: ...

【Report 層】
4. PostToolUse → post-fmt.sh
   → Edit/Write 後に format 実行、変更時のみ報告
   → 出力: [fmt] ... applied（整形不要時は無言）
5. Stop → stop-check.sh
   → セッション終了時に clippy/tsc チェック
   → 出力: [stop] WARN: ...（問題時のみ）

次のステップ:
1. まず `bash .claude/hooks/self-check.sh` を手で実行し、all healthy を確認
2. `/clear` でセッションをリセット
3. SessionStart(self-check → session-start) + Preflight Hook が動作することを確認
4. `/verify-setup` を実行して全構築物の整合性チェック
```
...
- [ ] `.claude/hooks/self-check.sh` が作成され、実行権限があり、SessionStart の先頭に登録されている
- [ ] `self-check.sh` を単体実行すると `[self-check] SUMMARY: ...` が出て、FAIL 時のみ終了コード 1 になる
- [ ] `.claude/settings.json` に Hook が設定され、全 hook が `"type": "command"` に統一されている
...
- [ ] 全 hook の出力が `[self-check]` / `[preflight]` / `[guard]` / `[fmt]` / `[stop]` プレフィックスで統一されている
- [ ] Grill me ステップ実施済み
- [ ] 進捗ファイルが更新されている
...
- ❌ 健康診断 `self-check.sh` を作らない / SessionStart 先頭に置かない（hook 単独エラーに気づけないまま運用してしまう）
- ❌ `self-check.sh` を FAIL 以外でも exit 1 にする（SessionStart をブロックしてセッションが始まらなくなる。FAIL 時のみ 1、WARN は 0）
