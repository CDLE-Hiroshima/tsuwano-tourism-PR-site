---
description: >
  Claude Code 環境構築の整合性を検証する。docs, CLAUDE.md / docs/agent-shared.md,
  .steering, skills, agents, commands, hooks のすべてが正しく構築され、相互参照が破綻して
  いないかを確認する。さらにライセンス整合検証 (proprietary plugin 導入時の同意ログ確認) を
  行い、検証レポートを生成する。
  /setup-hooks の完了後、または既存環境の健全性を確認したい時に実行する。
allowed-tools: Read, Glob, Grep, Bash(ls *), Bash(cat *), Bash(wc *), Bash(find *), Bash(grep *), Bash(jq *)
---

# /verify-setup — 環境構築の整合性検証

> Phase 8 of 8. Let's think step by step.
...
Phase 0-7 がすべて完了マークされている (または `[~] skipped` で意図的に skip されている) ことを確認。未完了のフェーズがあれば、そのフェーズに戻るよう通知。

skip 可能な Phase:

- Phase 2 (`/setup-marketplace`) skip: 公式 plugin 不使用。docs/external-skills.md は Phase 3 で空テンプレ生成済の前提

Phase 2 を skip した場合は公式 plugin 不使用構成として検証する (検証項目は減るが破綻はしない)。
...
# Phase 3: CLAUDE.md / docs/agent-shared.md
ls CLAUDE.md
...
### Step 2: CLAUDE.md / docs/agent-shared.md の品質チェック
...
#### docs/agent-shared.md (共通コンテキスト)

- [ ] 行数が 200 以下か
- [ ] プロジェクト概要 / アーキテクチャ要約 / 主要ディレクトリ / 規約サマリ / テスト方針 / git ワークフロー基本が記載されているか
- [ ] `@docs/external-skills.md` を 1 回参照しているか (公式 Skill リンク)
- [ ] Claude 固有事項が **混入していない** か (それは CLAUDE.md の責務)

2 ファイルを Read で読んで品質チェック。

### Step 2.5: ライセンス整合検証 (常時)

`docs/external-skills.md` を **単一の真実源** として、proprietary plugin の導入とライセンス同意ログの整合を検証する。

```bash
# proprietary plugin 同意ログの存在
...
> **設計原則**: `docs/external-skills.md` の表が proprietary plugin の唯一の真実源。proprietary plugin を導入した場合はライセンス同意ログが必須。
...
| 旧 (Phase 0-7 / 8 段階) | 新 (Phase 0-8 / 9 段階) | 内容 |
|---|---|---|
| Phase 0 | Phase 0 | bootstrap |
| Phase 1 | Phase 1 | docs |
| — | **Phase 2 (新規)** | marketplace (anthropics/skills) |
| Phase 2 | **Phase 3** | claude-md → CLAUDE.md / agent-shared.md |
| Phase 3 | **Phase 4** | skills |
| Phase 4 | **Phase 5** | agents |
| Phase 5 | **Phase 6** | commands |
| Phase 6 | **Phase 7** | hooks |
| Phase 7 | **Phase 8** | verify |
...
### Phase 3: CLAUDE.md / agent-shared.md [✅/⚠️/❌]
- CLAUDE.md 行数: [N] (上限 150)
- docs/agent-shared.md 行数: [N] (上限 200)
- `@docs/agent-shared.md` 参照: CLAUDE.md=[N]
- 必須セクション: ✅
...
- Agent → Command: ✅/❌
- Hook → Command: ✅/❌
- ライセンス整合 (external-skills.md ↔ proprietary 同意ログ): ✅/❌
...
`.steering/_setup-progress.md` の Phase 8 を完了マーク:

```markdown
- [x] **Phase 8: /verify-setup** — 整合性検証 (ライセンス整合 含む)
...
```
Phase 8 完了です。すべての構築フェーズが終わりました。
...
- [ ] ライセンス整合検証実施 (常時)
- [ ] 検証レポートが生成されている (新フェーズ番号 1-8 + 相互参照 + ライセンス整合検証)
- [ ] 検証レポートが .steering/ に保存されている
- [ ] 進捗ファイルが更新されている (Phase 8 完了マーク)
...
- ❌ ライセンス整合検証を skip する (proprietary plugin の同意ログ漏れは重大インシデント)
